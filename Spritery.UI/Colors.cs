...
public static class Colors
{
    ...
    public static readonly Color RebeccaPurple = new(102, 51, 153, 255);
    ...
    public static readonly Color Invisible = new(0, 0, 0, 0);
    ...
    public static Color FromArgb(string hex)
    {
        ...
        else if (hex.StartsWith("0x")) hex = hex[2..];

        if (hex.Length == "FFFFFF".Length)
        {
            return FromRgb(Convert.ToInt32(hex, 16));
        }

        if (hex.Length == "FFFFFFFF".Length)
        {
            return FromArgb(Convert.ToInt64(hex, 16));
        }

        return Invisible;
    }

    public static Color GetBestVisibleColor(Color color)
    {
        var bgRed = color.r;
        var bgGreen = color.g;
        var bgBlue = color.b;
        ...
        var bgLuminance = CalculateLuminance(bgRed, bgGreen, bgBlue);

        // Calculate the luminance of black and white
        var blackLuminance = CalculateLuminance(0, 0, 0);
        var whiteLuminance = CalculateLuminance(255, 255, 255);

        // Calculate the contrast ratios
        var contrastBlack = CalculateContrastRatio(blackLuminance, bgLuminance);
        var contrastWhite = CalculateContrastRatio(whiteLuminance, bgLuminance);

        // Determine the color with the highest contrast
        return contrastBlack > contrastWhite ? Black : White;
    }

    private static float CalculateLuminance(int red, int green, int blue)
    {
        var r = red / 255.0f;
        var g = green / 255.0f;
        var b = blue / 255.0f;
    ...
    private static float CalculateContrastRatio(float foregroundLuminance, float backgroundLuminance)
    {
        if (foregroundLuminance > backgroundLuminance)
        {
            return (foregroundLuminance + 0.05f) / (backgroundLuminance + 0.05f);
        }

        return (backgroundLuminance + 0.05f) / (foregroundLuminance + 0.05f);
    }
}
